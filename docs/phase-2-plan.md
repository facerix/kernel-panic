...
- `buildCrewMember(archetypeId, spawn, rng)` added to `src/game/archetypes/index.js`. Picks callsign from the archetype's `CALLSIGNS` list using `rng.nextInt(list.length)`. `buildPlayer` is deprecated but kept for backward-compat in the debug harness; removed in M2.
