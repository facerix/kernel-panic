...
#### CorpCivilian vs Lookout

Both can appear on the same map, but they are **not the same role**. `CorpCivilian` already exists as ambient map pressure; `Lookout` is a T2 combat specialist. Do not merge them.

| | `CorpCivilian` (ambient) | `Lookout` (T2 specialist) |
|---|---|---|
...
| **T2 — Specialist** | `ELEVATED` | fodder + **1 specialist** | Skirmisher, Guard + exactly one of {Sniper, Lookout, Medic} | Specialist at baseline; fodder count unchanged or +1 |
...
| ELEVATED | 2 fodder + lookout | `[Skirmisher, Guard, Lookout]` — kill lookout or keep breaking its LOS as it repositions |
...
- Wire `CorpCivilian` alarm checks to the new helper only. **Do not** change `Combat`, patrol-hostile acquisition, or lookout LOS — combat cover semantics (`COVER_HIT_PENALTY`, `COVER_DODGE_BONUS`) stay as-is. **Reuse the same helper in M4.3** for player → flanker concealment (inverted observer/target).
...
| Knob | Rule |
|------|------|
| **Tier band** | `STANDARD` → compact footprint; `ELEVATED` → medium; `CRITICAL` → large. Higher tier = more BSP leaves, longer routes, more room for sniper/lookout lanes. |
...
**Implementation order:** Lookout (M3.1) before Sniper (M3.2). Roster table above keeps Sniper before Lookout narratively; Lookout ships first because it reuses `PatrolHostile`, lands `ALARM` `kind` plumbing, and unlocks the T2 priority-target puzzle with existing fodder — Sniper's cross-turn telegraph is the higher-risk stretch.

#### M3.1 — Lookout

**Not `CorpCivilian`.** See [CorpCivilian vs Lookout](#corpcivilian-vs-lookout) — civilians stay ambient; lookouts are mobile combat specialists.

**Locked design (pre-implementation):**

| Axis | Decision |
|------|----------|
| Event plumbing | **`ALARM` with `kind` discriminator** — `World.raiseAlarm()` emits `kind: 'facility'`; lookout emits `kind: 'lookout'` directly (no `raiseAlarm()`). Patrol hostiles react to both kinds; lookout does not subscribe to `ALARM` (no self-wake / no civilian latch coupling). |
...
| Class shape | **`Lookout extends PatrolHostile`** — reuse patrol → investigate → engage pathing; override `bindToBus` to skip the alarm subscription (axis 3). `engageSteps` is spot-only (never attacks). |
| Vantage AI | **LOS + max Chebyshev distance** — when repositioning, prefer legal neighbour tiles that restore LOS while maximising distance to the target (skirmisher kiting inverted). **`LOOKOUT_SIGHT_RANGE = 10`** (baseline `SIGHT_RANGE` is 8; 12 is the playtest ceiling if 10 feels too short). |
| Cover / LOS | **Standard combat LOS** — cover does not block lookout sight (same as skirmisher acquisition). Cover alone cannot permanently neutralise a T2 specialist. |
| Stealth | **Hostile rules** — `acquireTarget` / `isSpottableBy`: stealthed targets visible only at Chebyshev ≤ 1. SLIDE remains a viable counter; lookout is not a range-stealth hard-counter like civilians. |
...
- **`Lookout extends PatrolHostile`.** Never attacks. ENGAGE branch = spot-only via `engageSteps`.
- **Spot mode:** each corp turn, if a hostile target is acquired (LOS + range + stealth rules), emit `EVENT.ALARM` with `{ kind: 'lookout', source, target, origin }`. Hostiles subscribed to `ALARM` force-ENGAGE on the shared target (both `facility` and `lookout` kinds); ping repeats every turn sight holds.
- **No LOS:** path toward last-known position or a vantage tile that restores LOS, scoring candidates by distance-maximising among tiles that hold sight. Smoke blocks LOS like any ranged check — lookout must move to a new angle.
- **Rep / facility alarm:** `kind: 'lookout'` pings do not trip facility latch or stack `REP.ALARM_PENALTY`. Facility alarm remains the civilian/terminal domain (`kind: 'facility'` only).
...
**Contrast with Lookout:** Lookout coordinates fodder (`ALARM` pings); sniper is a **self-contained personal threat** — break LOS or eat the burst. No ally force-multiplier.
...
| Range | **`SNIPER_SIGHT_RANGE = 12`** (baseline `SIGHT_RANGE` 8; lookout 10). |
