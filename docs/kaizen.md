...
- **Smoke charge LOS interaction.** The M4 smoke consumable introduces a `SMOKE` tile type that is passable but blocks LOS. The existing `Grid.blocksLineOfSight` and `hasLineOfSight` are tile-type checks; adding SMOKE should be a one-line constant addition. However, smoke is *temporary* (1 turn), which means either: (a) the grid tile is mutated and reverted after 1 turn, or (b) smoke is tracked as an overlay outside the grid. Option (a) is simpler but requires a "pending tile restoration" list in the World or Run; option (b) is cleaner but adds a new data structure. Decide at M4 implementation time.
