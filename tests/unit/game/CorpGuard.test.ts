...
test('CorpGuard is a corp-faction PatrolHostile with the guard glyph', () => {
  const guard = new CorpGuard({ id: 'guard-0', x: 1, y: 1 });
  assert.ok(guard instanceof PatrolHostile, 'shares the patrol state machine');
  assert.ok(!(guard instanceof CorpDrone), 'is a sibling of CorpDrone, not a subclass');
  assert.equal(guard.faction, FACTION.CORP);
  assert.equal(guard.glyph, 'g');
});

test('CorpGuard at T1 has baseline fodder stats: no armor, trades HP openly', () => {
  const guard = new CorpGuard({ id: 'guard-0', x: 1, y: 1, tier: ENEMY_TIER.T1 });
...
test('guard melees an adjacent target when AP allows', () => {
  ...
  const guard = new CorpGuard({ id: 'guard-0', x: 6, y: 2, maxAp: AP_COST.MELEE_ATTACK });
  w.addEntity(player);
  w.addEntity(guard);
  // roll 0.99 ≥ DODGE_CHANCE → connects.
  const log = guard.takeTurn(w, new StubRng([0.99]));
  assert.equal(guard.state, DRONE_STATE.ENGAGE);
...
test('guard closes the distance toward an out-of-reach target (no ranged option)', () => {
  ...
  const guard = new CorpGuard({ id: 'guard-0', x: 8, y: 2, maxAp: 4 });
  w.addEntity(player);
  w.addEntity(guard);
...
test('guard closes then strikes within a single turn (takeTurnSteps paces both)', () => {
  ...
  const guard = new CorpGuard({ id: 'guard-0', x: 6, y: 2, maxAp: 2 });
  w.addEntity(player);
  w.addEntity(guard);
...
test('guard does not melee a NEUTRAL bystander', () => {
  ...
  const guard = new CorpGuard({ id: 'guard-0', x: 6, y: 2, maxAp: 4 });
  w.addEntity(neutral);
  w.addEntity(guard);
  const log = guard.takeTurn(w, new Rng(1));
...
test('CorpGuard.unbind detaches the bus listeners', () => {
  const bus = new EventBus();
  const guard = new CorpGuard({ id: 'guard-0', x: 2, y: 2 });
  guard.bindToBus(bus);
  guard.unbind();
...
test('takeTurn on a dead guard is a no-op', () => {
  const w = openWorld();
  const guard = new CorpGuard({ id: 'guard-0', x: 1, y: 1, patrolWaypoints: [{ x: 5, y: 1 }] });
  w.addEntity(guard);
  guard.alive = false;
  const log = guard.takeTurn(w, new Rng(1));
...
test('CorpGuard constructor rejects malformed waypoints', () => {
  assert.throws(
    () => new CorpGuard({ id: 'guard-0', x: 1, y: 1, patrolWaypoints: [{ x: 0.5, y: 1 }] }),
    TypeError
  );
  assert.throws(
    () => new CorpGuard({ id: 'guard-0', x: 1, y: 1, patrolWaypoints: 'nope' }),
    TypeError
  );
});
